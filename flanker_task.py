...
expName = 'Flanker PSYC4007'  # from the Builder filename that created this script
expVersion = ''
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/eddychan/Desktop/4007/Flanker 4007/Flanker PSYC4007_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    First_instruction = visual.TextStim(win=win, name='First_instruction',
        text='Respond to the DIRECTION of the central arrow\n\nif you see…\nBLUE: Press the key matching the direction\nORANGE: Press the OPPOSITE key\n\nPress SPACE to begin',
        font='Arial',
        pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='white', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=0.0);
    First_instruction_key = keyboard.Keyboard(deviceName='defaultKeyboard')
    ...
    instructions = data.Routine(
        name='instructions',
        components=[First_instruction, First_instruction_key],
    )
    instructions.status = NOT_STARTED
    continueRoutine = True
    # update component parameters for each repeat
    # create starting attributes for First_instruction_key
    First_instruction_key.keys = []
    First_instruction_key.rt = []
    _First_instruction_key_allKeys = []
    ...
    while continueRoutine:
        ...
        # if First_instruction is starting this frame...
        if First_instruction.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            ...
            thisExp.timestampOnFlip(win, 'First_instruction.started')
            # update status
            First_instruction.status = STARTED
            First_instruction.setAutoDraw(True)
        
        # if First_instruction is active this frame...
        if First_instruction.status == STARTED:
            # update params
            pass
        
        # *First_instruction_key* updates
        waitOnFlip = False
        
        # if First_instruction_key is starting this frame...
        if First_instruction_key.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            First_instruction_key.frameNStart = frameN  # exact frame index
            First_instruction_key.tStart = t  # local t and not account for scr refresh
            First_instruction_key.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(First_instruction_key, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'First_instruction_key.started')
            # update status
            First_instruction_key.status = STARTED
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(First_instruction_key.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(First_instruction_key.clearEvents, eventType='keyboard')  # clear events on next screen flip
        if First_instruction_key.status == STARTED and not waitOnFlip:
            theseKeys = First_instruction_key.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
            _First_instruction_key_allKeys.extend(theseKeys)
            if len(_First_instruction_key_allKeys):
                First_instruction_key.keys = _First_instruction_key_allKeys[-1].name  # just the last key pressed
                First_instruction_key.rt = _First_instruction_key_allKeys[-1].rt
                First_instruction_key.duration = _First_instruction_key_allKeys[-1].duration
    ...
    # check responses
    if First_instruction_key.keys in ['', [], None]:  # No response was made
        First_instruction_key.keys = None
    thisExp.addData('First_instruction_key.keys',First_instruction_key.keys)
    if First_instruction_key.keys != None:  # we had a response
        thisExp.addData('First_instruction_key.rt', First_instruction_key.rt)
        thisExp.addData('First_instruction_key.duration', First_instruction_key.duration)
    thisExp.nextEntry()
    ...
    block = data.TrialHandler2(
        ...
        extraInfo=expInfo, 
        originPath=-1, 
        trialList=data.importConditions("blocks_"+expInfo['group']+".xlsx"), 
        seed=None, 
        isTrials=True, 
    )
    ...
    for thisBlock in block:
        ...
        thisExp.addData('block_intro.started', block_intro.tStart)
        ...
        thisExp.addData('block_intro.stopped', block_intro.tStop)
        ...
        trials = data.TrialHandler2(
            ...
            extraInfo=expInfo, 
            originPath=-1, 
            trialList=data.importConditions('4007/conditions.xlsx'), 
            seed=None, 
            isTrials=True, 
        )
        ...
        for thisTrial in trials:
            ...
            trial = data.Routine(
                name='trial',
                components=[fixation, key_resp, target_display],
            )
            ...
            key_resp.rt = []
            _key_resp_allKeys = []
            target_display.setColor(color, colorSpace='rgb')
            target_display.setText(stimulus)
            ...
            while continueRoutine:
                ...
                if target_display.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
                    # keep track of start time/frame for later
